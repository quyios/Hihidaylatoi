...
print("Running script version KR-ONLY V1.0")
...
def draw_ui(done, total, korea, status):
    ...
    print("Tiến trình tạo Proxy Korea ....\n")
...
def main():
    ...
    while len(proxies) < TOTAL_PROXY and not STOP_REQUEST:
        ...
        korea_total = count_all_projects(projects)
    ...
    tg_send_file(
        OUTPUT_FILE,
        f"{len(proxies)} Korea Proxy đã được tạo"
    )
